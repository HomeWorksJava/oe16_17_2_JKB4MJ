...
<body>
	...
	<c:forEach items="${tables}" var="table">
		...
		<c:if test="${table.booked}">
			<li>
				<label style="background-color:#ff0000">Table # ${table.number}</label>
			</li>
		</c:if>
		<c:if test="${!table.booked}">
			<li>
				<label style="background-color:#00ff00">Table # ${table.number}</label>
	...
	<c:forEach items="${reservations}" var="res">
		<li>Table # ${res.tableNumber} is reserved by ${res.name}</li>
